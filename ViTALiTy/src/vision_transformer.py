...
class Attention(nn.Module):
    ...
    def _taylor_1_degree(self, q, k, v):
        kv = k.transpose(-2, -1) @ v
        out = q @ kv
        return out

    def _taylor_2_degree(self, q, k, v):
        B, H, N, Ch = q.shape
        q2 = self._l2_normalize(q, self.eps)
        k2 = self._l2_normalize(k, self.eps)
        q2 = q2 * q2
        k2 = k2 * k2
        q2 = self._l2_normalize(q2, self.eps)
        k2 = self._l2_normalize(k2, self.eps)

        ones_qk = torch.ones((B, H, N, 1), device=q.device, dtype=q.dtype)
        q2 = torch.cat([q2, ones_qk], dim=-1)
        k2 = torch.cat([k2, ones_qk], dim=-1)
        v2 = pad(v, (0, 1), value=1.0)

        kv2 = k2.transpose(-2, -1) @ v2
        out2_full = q2 @ kv2

        num2 = out2_full[..., :-1]
        den2 = out2_full[..., -1:].clamp_min(self.eps)
        out = num2 / den2
        return out
    ...
    def forward(self, x):
        ...
        if self.vitality:
            ...
            if self.degree == 1:
                taylor_out = self._taylor_1_degree(q, k, v)
            else:
                taylor_out = self._taylor_2_degree(q, k, v)
                taylor_out = self.alpha * taylor_out

            attn = (sparse @ v + taylor_out) * self.scale
